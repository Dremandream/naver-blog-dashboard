...
## 마지막 업데이트
2026-07-21 (세션 13, Claude Code CLI / Fable 5)
...
- **김찰저 제거**: §1 참조. 재검토 결과 코드 버그 아님 — kimcharger 채널이 t.me/s 웹프리뷰를 꺼서(HTTP 200이지만 메시지 0개, 07-14 이후 미수집) 표본 부족으로 "결과 대기"였던 것. 사용자 지시로 채널 제거. 창 확장과 상충(긴 창일수록 최근 소스 늦게 표시)했으나 제거로 해소됨. **알려진 취약점(미수정)**: `fetchTelegramChannel`은 HTTP 200이면 메시지 0개여도 경고 없이 넘어감 → 다른 채널이 같은 이유로 조용히 죽을 수 있음. 필요 시 0건 경고 로그 추가 고려.
