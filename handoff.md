...
- **미결/제안(사용자 검토 요청)**: ⓐ순위 기준을 3개월(현재)→1년으로? 사용자 의도가 장기평가라 1년 정렬이 더 맞을 수 있으나 대기 소스가 하위로 밀림 — 미적용, 검토 필요. ⓑ랭킹 신뢰용 MIN_SAMPLE 5→20~30? 1년에 n=12~32 소수 소스 있음 — 미적용. ⓒ history 17.5MB 여전히 큼 → 종가를 "필요 셀만"(의견일+평가시점) 남기는 정밀 트림하면 대폭 감소 가능하나 위험/복잡 — 미적용.
